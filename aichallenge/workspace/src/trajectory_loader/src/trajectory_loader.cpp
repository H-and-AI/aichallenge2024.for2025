...
{
  pub_trajectory_ = create_publisher<Trajectory>("/my_topic/trajectory", rclcpp::QoS(1));
...
void TrajectoryLoaderNode::load_path(const std::string & file_path)
{
  ...
  while (std::getline(file, line)) {
    ...
    std::string x, y, longitudinal_velocity_mps, dummy;
    ...
    point.pose.position.x = std::stod(x); //原点のオフセット;
